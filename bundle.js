var quarantine = (function (exports) {
  ...
  function transpose(matrix) {
    ...
    for (var i = -1, m = min$1(matrix, length$3), transpose = new Array(m); ++i < m;) {
  ...
  function dragDisable$1(view) {
    var root = view.document.documentElement,
        selection = select(view).on("dragstart.drag", noevent$4, true);
  ...
  function longitude(point) {
    if (abs$1(point[0]) <= pi$1)
      ...
      return sign$2(point[0]) * ((abs$1(point[0]) + pi$1) % tau$1 - pi$1);
  }
  ...
  function clipAntimeridianLine(stream) {
    ...
    return {
      ...
      point: function(lambda1, phi1) {
        ...
        if (abs$1(delta - pi$1) < epsilon$3) { // line crosses a pole
      ...
      constructor(level, renderFunction, victoryCallback, debugInfo) {
          ...
          this.simulation = simulation()
              ...
              if (wallPolys.length > 0) {
                  const wallMultiPoly = index.union(wallPolys[0], ...wallPolys.slice(1));
